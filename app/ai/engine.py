...
class AIEngine:
    ...
    async def analyze_message(self, text: str, *, trace_id: str) -> AIResult:
        await self.event_bus.publish(
            Event(EventNames.AI_REQUEST_STARTED, {"kind": "analysis"}, trace_id)
        )
        result = await self.provider.generate(
            [AIMessage("system", GHOST_SYSTEM_PROMPT), AIMessage("user", text)], trace_id=trace_id
        )
        await self.event_bus.publish(
            Event(
                EventNames.AI_RESPONSE_GENERATED,
                {"kind": "analysis", "model": result.model},
                trace_id,
            )
        )
        return result
    ...
    async def draft_reply(
        ...
        *,
        ...
        user_content = "\n".join(context) + f"\n\nIncoming message:\n{text}"
        await self.event_bus.publish(
            Event(EventNames.AI_REQUEST_STARTED, {"kind": "reply"}, trace_id)
        )
        result = await self.provider.generate(
            [AIMessage("system", REPLY_SYSTEM_PROMPT), AIMessage("user", user_content)],
            trace_id=trace_id,
        )
        await self.event_bus.publish(
            Event(
                EventNames.AI_RESPONSE_GENERATED, {"kind": "reply", "model": result.model}, trace_id
            )
        )
        return result
    ...
    async def answer_command(self, query: str, context: str, *, trace_id: str) -> AIResult:
        return await self.provider.generate(
            [
                AIMessage("system", COMMAND_SYSTEM_PROMPT),
                AIMessage("user", f"Context:\n{context}\n\nOwner request:\n{query}"),
            ],
            trace_id=trace_id,
        )
