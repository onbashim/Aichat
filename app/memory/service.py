...
class MemoryService:
    ...
    async def create(
        self, *, chat_id: int | None, memory_type: str, key: str, value: str, importance: int = 0
    ):
        memory = await self.repository.create_memory(
            chat_id=chat_id, memory_type=memory_type, key=key, value=value, importance=importance
        )
        await self.event_bus.publish(
            Event(EventNames.MEMORY_CREATED, {"memory_id": str(memory.id)})
        )
        return memory
