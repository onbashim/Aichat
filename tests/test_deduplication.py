...
class NeverCalled:
    def __getattr__(self, name):
        async def fail(*args, **kwargs):
            raise AssertionError(f"{name} should not be called for duplicate update")
...
async def test_duplicate_update_is_not_processed():
    service = TelegramUpdateService(
        ...
        telegram=NeverCalled(),
        event_bus=NeverCalled(),
        orchestrator=NeverCalled(),
        command_center=NeverCalled(),
        owner_auth=OwnerAuthenticationMiddleware(owner_id=1, rate_limiter=NeverCalled()),
    )
    repo = FakeRepo()
    await service.process(repo, ParsedUpdate(99, "message", {"text": "hello"}))
    assert repo.calls == 1
